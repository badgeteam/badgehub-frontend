...
export function Filter({categories, devices}: {categories: Category[], devices: Device[]}) {
    ...
    return <section className={styles.appFilter}>
        Filter:
        <select name="categories" id="categories" onChange={setUrlParams} ref={inputCategoryRef} defaultValue={params.get('category') || ""}>
        ...
        <select name="devices" id="devices" onChange={setUrlParams} ref={inputDeviceRef} defaultValue={params.get('device') || ""}>
        <option value="" key="all">All</option>
